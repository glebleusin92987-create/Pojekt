...
def plot_all_s_params_raw_separate(freq, S11, S21, S12, S22, title_prefix):
    """
    График всех исходных S-параметров.
    """
    # График амплитуд
...
def read_s2p_file(filename):
    """Чтение файла s2p в формате dB/угол."""
    freq = []
    s11_mag, s11_deg = [], []
    s21_mag, s21_deg = [], []
    s12_mag, s12_deg = [], []
...
def stepwise_nrw_method(S11, S21, freq_Hz, d, fc):
    """
    Устойчивая реализация пошагового метода NRW.
    """
    c = 299792458.0
    ...
    # 1. ВЫЧИСЛЕНИЕ ВОЛНОВОГО СОПРОТИВЛЕНИЯ z (формула 10)
    numerator = (1 + S11) ** 2 - S21 ** 2
    denominator = (1 - S11) ** 2 - S21 ** 2
    with np.errstate(invalid='ignore', divide='ignore'):
        z = np.sqrt(numerator / denominator)
    z = np.where(z.real >= 0, z, -z)  # Условие пассивности

    # 2. ВЫЧИСЛЕНИЕ exp(gamma*d) (формула 11)
    with np.errstate(invalid='ignore', divide='ignore'):
        term1 = (1 - S11 ** 2 + S21 ** 2) / (2 * S21)
        z_term = z - 1 / z
        # Избегаем деления на ноль в z_term
    ...
    # 3. УСТОЙЧИВОЕ ОПРЕДЕЛЕНИЕ НАЧАЛЬНОЙ ВЕТВИ m0
    # Используем оценку через волновое сопротивление и условие d < λ/2 на первой точке
    lambda_min = c / freq_Hz[0]
    if d < lambda_min / 2:
        m0 = 0
    else:
        # Более надежная оценка: используем фазовый сдвиг, вычисленный из z и грубой оценки n
        # Для диэлектрика без магнитных потерь n ≈ sqrt(ε_r) ≈ Re(z) при μ_r ≈ 1
        beta_est = k0[0] * np.sqrt(3 * 1)
        m0 = int(np.round((beta_est * d) / (2 * np.pi)))
        # Гарантируем, что m0 >= 0 для пассивной среды
        m0 = max(m0, 0)

    phi_corrected = np.zeros_like(phi_raw)
    phi_corrected[0] = phi_raw[0] + 2 * np.pi * m0

    # 4. ПОШАГОВАЯ РАЗМОТКА С ФИЛЬТРАЦИЕЙ АНОМАЛЬНЫХ СКАЧКОВ
    for i in range(1, n_points):
        delta_phi = phi_raw[i] - phi_raw[i - 1]
        # ЖЕСТКАЯ ФИЛЬТРАЦИЯ: если |Δφ| >= π, это ошибка измерения/шум. Интерполируем.
        if np.abs(delta_phi) >= np.pi:
            # Линейная интерполяция фазы от предыдущей корректной точки
            # Это предпочтительнее, чем принудительное добавление 2π
            if i >= 2:
    ...
    # 5. ВЫЧИСЛЕНИЕ ПАРАМЕТРОВ МАТЕРИАЛА
    omega_c = 2 * np.pi * fc
    valid = freq_Hz > fc  # Только выше критической частоты
    ...
    mu_r[valid] = z[valid] * root_term[valid]
    with np.errstate(invalid='ignore'):
        eps_r[valid] = n_sq[valid] / mu_r[valid]

    # 6. ПОСЛЕДУЮЩАЯ ФИЛЬТРАЦИЯ: удаление выбросов по производной
    # Рассчитываем производную ε' по частоте
    if np.any(valid):
        eps_real_valid = eps_r[valid].real
        freq_valid = freq_Hz[valid]
        deriv = np.abs(np.gradient(eps_real_valid, freq_valid))
        median_deriv = np.median(deriv)
        # Помечаем как выбросы точки, где производная > 10 * медианной
        outlier_mask = deriv > (10 * median_deriv)
        if np.any(outlier_mask):
            # Заменяем выбросы интерполированными значениями
            from scipy import interpolate
            x = freq_valid[~outlier_mask]
            y = eps_real_valid[~outlier_mask]
            if len(x) > 3:
                f_interp = interpolate.interp1d(x, y, kind='linear', bounds_error=False, fill_value='extrapolate')
                eps_real_valid[outlier_mask] = f_interp(freq_valid[outlier_mask])
                eps_r[valid].real = eps_real_valid

    T_corrected = A * np.exp(1j * phi_corrected)
    return eps_r, mu_r, z, T_corrected


def calculate_for_frequency_range(filename, sample_length_m, waveguide_width_m, freq_min_ghz=None, freq_max_ghz=None):
    """
    Основная функция расчета с выбором диапазона частот.
    """
    # Чтение данных
    freq, S11, S21, S12, S22 = read_s2p_file(filename)

    # Визуализация исходных S-параметров
    plot_all_s_params_raw_separate(freq, S11, S21, S12, S22, "Исходные данные")
    ...
    # Создание маски для выбранного диапазона
    freq_mask = (freq >= freq_min_hz) & (freq <= freq_max_hz)

    freq_selected = freq[freq_mask]
    S11_selected = S11[freq_mask]
    S21_selected = S21[freq_mask]
    S12_selected = S12[freq_mask]
    S22_selected = S22[freq_mask]
    ...
    # Визуализация сравнения до и после деэмбеддинга
    ...
    # Усреднение для симметричного образца
    S11_avg = (S11_corr + S22_corr) / 2
    S21_avg = (S21_corr + S12_corr) / 2

    # Расчет параметров материала
    eps_r, mu_r, z, T_corrected = stepwise_nrw_method(
        S11_corr, S21_corr, freq_selected, sample_length_m, fc
    )

    # Фильтрация валидных точек
    valid_mask = (freq_selected > fc)  # Только выше критической частоты

    print(f"\nВалидных точек (выше f_c): {np.sum(valid_mask)} из {len(freq_selected)}")

    if np.any(valid_mask):
        eps_real = eps_r[valid_mask].real
        eps_imag = -eps_r[valid_mask].imag
        mu_real = mu_r[valid_mask].real
        mu_imag = -mu_r[valid_mask].imag
    ...
    # 1. S-параметры (амплитуда) после усреднения
    ax1 = axes[0, 0]
    ...
    # 2. S-параметры (фаза) после усреднения
    ax2 = axes[0, 1]
    ...
    # 3. Диэлектрическая проницаемость (действительная)
    ax3 = axes[1, 0]
    if np.any(valid_mask):
        ax3.plot(freq_selected[valid_mask] / 1e9, eps_r[valid_mask].real, 'b-',
                 label="ε'", linewidth=2, alpha=0.8, marker='o', markersize=4)
        mean_eps = np.mean(eps_r[valid_mask].real)
    ...
    # 4. Диэлектрическая проницаемость (мнимая)
    ax4 = axes[1, 1]
    if np.any(valid_mask):
        ax4.plot(freq_selected[valid_mask] / 1e9, -eps_r[valid_mask].imag, 'r-',
                 label="ε''", linewidth=2, alpha=0.8, marker='s', markersize=4)
        mean_eps_imag = np.mean(-eps_r[valid_mask].imag)
    ...
    # 5. Магнитная проницаемость (действительная)
    ax5 = axes[2, 0]
    if np.any(valid_mask):
        ax5.plot(freq_selected[valid_mask] / 1e9, mu_r[valid_mask].real, 'g-',
                 label="µ'", linewidth=2, alpha=0.8, marker='^', markersize=4)
        mean_mu = np.mean(mu_r[valid_mask].real)
    ...
    # 6. Магнитная проницаемость (мнимая)
    ax6 = axes[2, 1]
    if np.any(valid_mask):
        ax6.plot(freq_selected[valid_mask] / 1e9, -mu_r[valid_mask].imag, 'm-',
                 label="µ''", linewidth=2, alpha=0.8, marker='v', markersize=4)
        mean_mu_imag = np.mean(-mu_r[valid_mask].imag)
